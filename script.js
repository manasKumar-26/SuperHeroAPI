...
var arr = []; // array to store the id of our fav super hero
/*This function is used to give the  downmenu when something is seaerched */
function getName() {
  ...
  xhr.open(
    "get",
    `https://www.superheroapi.com/api.php/2163050803820238/search/${char}`,
    true
  );
  xhr.send();
}
...
function displayHero(e) {
  ...
  xhrreq.open(
    "get",
    `https://www.superheroapi.com/api.php/2163050803820238/search/${nameOfHero}`,
    true
  );
  xhrreq.send();
}
...
function displayImage(id, name, power, bio) {
  ...
  xhrRequest.onload = function () {
    ...
    document.getElementById(
      "universe"
    ).innerText = `Published By: ${bio.publisher}`;
    document.getElementById("nameofhero").innerText = `Avatar Name: ${name}`;
    document.getElementById(
      "favbutton"
    ).innerHTML = `<button class="btn btn-info my-2 my-sm-0" type="submit" id="favbut">Add To My Team</button>`;
    document.getElementById(
      "teambut"
    ).innerHTML = `<button class="btn btn-info my-2 my-sm-0" type="submit" id="gototeampage">Goto My Team</button>`;
  ...
  xhrRequest.open(
    "get",
    `https://www.superheroapi.com/api.php/2163050803820238/${id}/image`,
    true
  );
  xhrRequest.send();
}
...
function detailedimagepage() {
  window.open(`index2.html#${id1}`, "_blank");
}
...
function createString() {
  ...
  /* if(localStorage.getItem('fav')!==null)
    ...
    {
        var a1=[str];
        localStorage.setItem('fav',JSON.stringify(a1));
    }*/
...
function teampage() {
  window.open(`index3.html#${res}`, "_blank");
}
