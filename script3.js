...
for (
  ...
  i++ // first 2 elements are link and count so ignore
) {
  ...
  xhr.onload = function () {
    ...
    imagelem.addEventListener("click", function () {
      //open then detailed info about hero wherre we hovered
      window.open(`index2.html#${id}`, "_blank");
    });
  ...
  xhrreq.open(
    "get",
    `https://www.superheroapi.com/api.php/2163050803820238/${id}`,
    false
  );
  xhrreq.send();
